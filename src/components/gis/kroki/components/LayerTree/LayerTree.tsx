...
export default function LayerTree({ mapRef, onOpenStyle, onOpenLayerModal }: LayerTreeProps) {
  ...
  const collectDescendantLayerIds = (folderId: string): string[] => {
  ...
  function handleExport(layerId: string, format: string) {
    exportLayerAs(layerId, format, layers, features, JSZip).then((res) => showToast(res.msg));
  }

  function openExportMenu(x: number, y: number, layerId: string) {
    setCtxMenu({
      x, y,
      items: [
  ...
  function FolderRow({ folder, depth }: { folder: any; depth: number }) {
  ...
  function LayerRow({ lyr, depth }: { lyr: any; depth: number; key?: any }) {
  ...
  function renderLevel(parentFolderId: string | null, depth: number): React.ReactNode {
