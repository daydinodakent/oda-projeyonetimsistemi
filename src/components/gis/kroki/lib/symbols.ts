...
export const SYMBOL_LABELS: Record<SymbolKey, string> = {
  circle: 'Daire', pin: 'İğne', star: 'Yıldız', triangle: 'Üçgen',
  square: 'Kare', home: 'Ev', flag: 'Bayrak', cross: 'Çarpı'
};
...
export function symbolIconSvg(key: SymbolKey): string {
  return `<svg viewBox="0 0 24 24" fill="currentColor" stroke="none">${SYMBOL_SVG[key] || SYMBOL_SVG.circle}</svg>`;
}

function drawSymbolShape(ctx: CanvasRenderingContext2D, key: SymbolKey) {
...
export function createSymbolIcon(key: SymbolKey): ImageData {
  const size = 32;
...
export function ensureSymbolIcons(map: any) {
...
export type SnapKey = typeof SNAP_TYPES[number];

function drawSnapShape(ctx: CanvasRenderingContext2D, type: SnapKey, c: number) {
  ctx.beginPath();
  if (type === 'vertex') {
...
function createSnapIcon(type: SnapKey): ImageData {
  const size = 26;
  const canvas = document.createElement('canvas');
  canvas.width = size; canvas.height = size;
  const ctx = canvas.getContext('2d');
...
export function ensureSnapIcons(map: any) {
  try {
...
export const GEOM_TYPE_SVG: Record<string, string> = {
...
export function geomTypeIcon(t: string | null): string {
  const d = (t && GEOM_TYPE_SVG[t]) || '<circle cx="8" cy="8" r="3"/><path d="M13 19l6-6M13 13h6v6"/>';
  return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">${d}</svg>`;
}

export function geomTypeLabel(t: string | null): string {
