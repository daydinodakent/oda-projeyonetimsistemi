...
function coordToKml(c) { return c.join(',') + ',0'; }

export function featuresToKML(feats) {
...
// Yerel metre koordinat sistemine (ilk noktaya göre) yaklaşık dönüşümle DXF üretir.
export function featuresToDXF(feats) {
  ...
  const toXY = (c) => [(c[0] - olng) * mPerDegLng, (c[1] - olat) * mPerDegLat];
...
function writeDoubleLE(view, offset, val) { view.setFloat64(offset, val, true); }

// shapeType: 1=Point, 3=PolyLine, 5=Polygon
export function buildShapefileBuffers(feats, shapeType) {
  ...
  let contentLen = 0;
  const recBuffers = records.map((r, i) => {
    let bodyLen;
    if (shapeType === 1) bodyLen = 20;
    else {
      const totalPoints = r.parts.reduce((s, p) => s + p.length, 0);
    ...
    if (shapeType === 1) {
      ...
      let idx = 0;
      r.parts.forEach((p) => { writeInt32LE(view, o, idx); o += 4; idx += p.length; });
      allPts.forEach((p) => { writeDoubleLE(view, o, p[0]); o += 8; writeDoubleLE(view, o, p[1]); o += 8; });
    }
...
export function buildDBF(feats) {
...
export async function exportLayerAs(layerId, format, layers, features, JSZip) {
  ...
  if (format === 'shp') {
    ...
    const names = { 1: 'points', 3: 'lines', 5: 'polygons' };
    const zip = new JSZip();
    let any = false;
    for (const key of Object.keys(groups)) {
      ...
      any = true;
      const built = buildShapefileBuffers(arr, parseInt(key, 10));
